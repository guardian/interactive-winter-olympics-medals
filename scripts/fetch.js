...
const generateMedalsTable = async() => {
    const data = await rp({ uri: "http://api.stats.com/v1/stats/oly/wntr_oly/medals/?accept=json&api_key=gmqfer9bzzufxr2w84v52xqt&sig=3d6c4719d61d8b23edcbba94904f93fc2fad921cd6e6486444b923d590063c5a", json: true });

    const medalsData = data.apiResults[0].league.medals;
...
const loadData = (disciplineCodes) => {
    return new Promise((resolve, reject) => {
        async.map(disciplineCodes, async.asyncify(async(sport) => {
            ...
            const response = await rp({ "uri": `http://api.stats.com/v1/stats/oly/wntr_oly/${sport}/medals/?accept=json&api_key=gmqfer9bzzufxr2w84v52xqt`, "json": true });
            console.log("medals:" + sport + " ✓")
            return response;
        }), (err, results) => {
            if (err) {
...
const loadScheduleData = (disciplineCombinations) => {
    return new Promise((resolve, reject) => {
        async.map(disciplineCombinations, async.asyncify(async(sportArr) => {
            ...
            const response = await rp({ "uri": `http://api.stats.com/v1/stats/oly/wntr_oly/${sportArr[0]}/events/?date=${sportArr[1]}&api_key=gmqfer9bzzufxr2w84v52xqt`, "json": true });
            console.log("schedule: " + sportArr[0] + " on " + sportArr[1] + " ✓")
            return response;
        }), (err, results) => {
            if (err) throw err;
...
const generator = async() => {
    ...
    const scheduleResp = await rp({uri: "http://api.stats.com/v1/stats/oly/wntr_oly/schedule/?accept=json&api_key=gmqfer9bzzufxr2w84v52xqt&sig=b340ae69d5e6ba22c7a9dc1e9afab79ba24c210c39544ff423f4e2036fea7a94", json: true});
